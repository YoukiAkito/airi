...
export function createAlayaMemoryContext(): ContextMessage | null {
  ...
  // Prefer the relevance-ranked results that `prepareContext()` produced
  // before this turn — see the sync/async constraint noted in the header.
  // When prefetch yielded nothing (storage error, or not run yet) degrade
  // quietly to the most recently accessed entries.
  const prepared = alaya.preparedMemories
  const ranked = prepared
    ? prepared.map(r => r.entry)
    : [...alaya.allMemories]
  ...
  const lines = ranked.slice(0, 10).map((m, i) => {
 ...
 */
export interface RegisterAlayaAutoIngestionOptions {
  /**
   * Resolves the session id used to tag short-term turns.
...
export function registerAlayaAutoIngestion(
  ...
  options: RegisterAlayaAutoIngestionOptions = {},
): () => void {
  const alaya = useAlayaMemoryStore()

  return onChatTurnComplete(async (chat: any, context: any) => {
    ...
    const sessionId = options.resolveSessionId?.() ?? 'default'

    // Record the assistant side of the exchange. The user side is recorded
    // in `chat.ts#ingest()`, the only place that knows the target session
    // before the runtime resolves one.
    const outputText = typeof chat?.outputText === 'string' ? chat.outputText.trim() : ''
    if (outputText) {
      try {
        alaya.addTurn({ role: 'assistant', content: outputText, sessionId })
      }
      catch { /* best-effort: memory must never break the turn */ }
    }
